...
static void write_to_io_mem(intcode_io_mem_t* const storage,
                            const int64_t value);
...
int set_mem_value(intcode_t* const prog,
...
intcode_t* copy_intcode(const intcode_t* const prog)
{
    ...
    if (prog != NULL)
    {
        ...
        {
            int64_t* memory_copy =
                (int64_t*) malloc(sizeof(int64_t) * prog->memory_size);
            if (memory_copy != NULL)
            {
                for (size_t i = 0; i < prog->memory_size; ++i)
                {
...
intcode_io_mem_t* create_io_mem()
{
    ...
    if (store != NULL)
    {
        store->value    = 0;
        store->consumed = 1;
        pthread_mutex_init(&store->mut, NULL);
    ...
    if (prog != NULL)
    {
        ...
        *op_code    = get_opcode(get_mem_value(prog, prog->head));
        ...
        else if (is_valid_opcode(*op_code))
        {
            ...
            get_parameter_modes(
                get_mem_value(prog, head), inst_size - 1, parameter_modes);
            if (get_parameter_values(prog,
                                     inst_size - 1,
                                     store_param,
                                     parameter_modes,
                                     parameters))
...
int input_op(intcode_t* const prog, const int64_t* const parameters)
{
    ...
    if ((prog != NULL) && (parameters != NULL))
    {
        ...
        else if (prog->io_mode == INT_CODE_MEM_IO)
        {
            ...
            {
                pthread_cond_wait(&prog->mem_io_in->cond,
                                  &prog->mem_io_in->mut);
            }
    ...
    if (parameters != NULL)
    {
        ...
        else if (prog->io_mode == INT_CODE_MEM_IO)
        {
            ...
            while (!prog->mem_io_out->consumed)
            {
                ...
                pthread_cond_wait(&prog->mem_io_out->cond,
                                  &prog->mem_io_out->mut);
            }
    ...
    if ((prog != NULL) && (parameter_modes != NULL) && (parameters != NULL))
    {
        ...
        for (size_t i = 0; i < num_parameters; i++)
        {
            ...
            if (parameter_modes[i] == PARAM_MODE_POSITION)
            {
                ...
                {
                    /*The parameter value is the address where the result of op
                     * is stored.*/
            ...
            else if (parameter_modes[i] == PARAM_MODE_RELATIVE)
            {
                ...
                else
                {
                    /*The parameter value is stored at the address*/
                    param_val =
                        get_mem_value(prog, memory_val + prog->relative_base);
                }
            }
            else
            {
